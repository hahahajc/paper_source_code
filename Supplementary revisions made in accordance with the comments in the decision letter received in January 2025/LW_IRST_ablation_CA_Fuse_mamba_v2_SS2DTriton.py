...
class LW_IRST_ablation_CA_Fuse_mamba_v2_SS2DTriton(nn.Module):
    ...
    def __init__(self, n_classes=1, encoder_relu=False, decoder_relu=True, channel=(8, 32, 64), dilations=(2,4,8,16), kernel_size=(7,7,7,7), padding=(3,3,3,3)):
        ...
        self.Depthwise4_1 = RegularBottleneck(
            channel[2], padding=1, depthwise=True, dropout_prob=0.1, relu=encoder_relu)
        self.Atrous4_2 = RegularBottleneck(
            channel[2], dilation=dilations[2], padding=dilations[2], dilated=True, dropout_prob=0.1, relu=encoder_relu)
        self.Asymmetric4_3 = RegularBottleneck(
            ...
            dropout_prob=0.1,
            relu=encoder_relu)
        self.Atrous4_4 = RegularBottleneck(
            channel[2], dilation=dilations[3], padding=dilations[3], dilated=True, dropout_prob=0.1, relu=encoder_relu)
...
#---------------------------------------------------------------------------------------------
        x3 = self.Depthwise4_1(x3)    # 1 64 32 32
        x3 = self.Atrous4_2(x3)       # 1 64 32 32
        x3 = self.Asymmetric4_3(x3)   # 1 64 32 32
        x3 = self.Atrous4_4(x3)       # 1 64 32 32
...
if __name__ == '__main__':
    ...
    model = LW_IRST_ablation_CA_Fuse_mamba_v2_SS2DTriton(channel=(8, 32, 64), dilations=(2,4,8,16), kernel_size=(7,7,7,7), padding=(3,3,3,3)).cuda() # kernel_size/padding = 5/2 7/3 9/4
